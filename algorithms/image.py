...
def james_image_data(path= Path("static/assets/jamesimages/"), img_list=None):  # change to jamesimages
    if img_list is None:  # color_dict is defined with defaults
        img_list = [
...
def planner_image_data(path= Path("static/assets/plannerimages/"), img_list=None):  # change to planner event images
    if img_list is None:  # color_dict is defined with defaults
        img_list = [ # dictionary used to store data values in key: value pairs
            {'source': "Birthday", 'label': "Birthday is in Month", 'file': "birthday.png"},
            {'source': "Urgent", 'label': "Urgent Task", 'file': "urgent.png"},
            {'source': "Exam", 'label': "Exam or Test", 'file': "exam.png"}
        ]
    # gather analysis data and meta data for each image, adding attributes to each row in table
    for img_dict in img_list:       # lists and dictionary
        file = path / img_dict['file']  # file with path for local access (backend)
        # Python Image Library operations
        img_reference = Image.open(file)  # PIL (Python image library) opens file
        img_data = img_reference.getdata()  # Reference https://www.geeksforgeeks.org/python-pil-image-getdata()
                                            # get image data()
        img_dict['format'] = img_reference.format
        img_dict['mode'] = img_reference.mode
        ...
        img_dict['base64'] = image_formatter(img_reference, img_dict['format'])
        # Numpy is used to allow easy access to data of image, python list
        img_dict['data'] = numpy.array(img_data) # taking image data and putting it into the dictionary
        img_dict['hex_array'] = []
        img_dict['binary_array'] = []
        # 'data' is a list of RGB data, the list is traversed and hex and binary lists are calculated and formatted
        for pixel in img_dict['data']: # original image data
            # hexadecimal conversions
            hex_value = hex(pixel[0])[-2:] + hex(pixel[1])[-2:] + hex(pixel[2])[-2:]
            hex_value = hex_value.replace("x", "0")
            img_dict['hex_array'].append("#" + hex_value)
            ...
            img_dict['binary_array'].append(bin_value)
        # create gray scale of image, ref: https://www.geeksforgeeks.org/convert-a-numpy-array-to-an-image/
        img_dict['gray_data'] = [] # grayscale data
        for pixel in img_dict['data']: # putting grayscale data into dictionary
            average = (pixel[0] + pixel[1] + pixel[2]) // 3
            if len(pixel) > 3:
                img_dict['gray_data'].append((average, average, average, pixel[3]))
            else:
